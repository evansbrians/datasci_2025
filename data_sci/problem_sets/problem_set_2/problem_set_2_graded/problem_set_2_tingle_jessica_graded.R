...
coqui_long %>% 
  distinct(habitat, transect_id) %>% 
  count(
    habitat,
    name = "n_transects")

# [[-0.05]] Code formatting: If a function spans more than one line of code, the
# closing parentheses should be followed placed on their own line.
...
# The second normal is violated

# That became clear during exploration of duplicate rows

# It was also kind of obvious looking at the column names that some attributes pertain to locations,
# whereas other attributes pertain to observations

# We really should have two tibbles, by Codd! ... :)!
# * One with transect_id as the primary key, where site, longitude, latitude, and habitat are attributes
# * One with transect_id, distance_class, and date as a compound primary key, where count is the attribute
