...
read_rds('data/raw/dc_birds.rds') %>% 
  tidyselect:::select(visits, birds, measures) %>%  
...
banding <-
  ...
  # Change measures spp column name to 'species' to match birds:
  ...
  # Join measures and birds by spp to get common names:
  ...
  # Select desired columns and their order:
...
banding_subset <- 
  ...
  # Filter observations greater than 100, grouped by common name:
  
  filter(
    n() >= 100,
    .by = common_name,
    
    # Filter age and sex for known values:
    
    if_all(
      age:sex,
      ~ .x != 'U'),
    
    # Filter wing and mass values:
    
    wing > 40,
    mass > 7) %>% 
  
  # Collapse age factors into adult and juvenile:
...
banding_subset %>% 
  ...
  map(
    ...
    # Body specifying species and mean mass:
    
    function(x) {
      banding_subset %>% 
        
        # Split data by species and calculate mean mass:
...
# Generate a character vector of the files with bird and .rds:
...
list.files(
  ...
  # read_rds on all files in vector
  ...
  # assign list names to global environment:
