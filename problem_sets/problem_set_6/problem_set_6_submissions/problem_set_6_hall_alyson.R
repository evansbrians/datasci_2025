...
# distinct()# In a separate code block from the above, remove the name `banding` from 
...
# mean_species_var(spp = "Northern Cardinal",
#                  measure = "wing")
...
banding_subset %>% 
  ...
  map(
    ~ banding_subset %>% 
      ...
      pull(mass) %>% 
      mean(na.rm = TRUE)) %>% 
  setNames(banding_subset %>% 
             pull(common_name) %>% 
             unique()) %>% 
  bind_rows()

          
# I am assuming I cannot use map_dfr() or tibble() since they aren't listed
# so this is as close as I can get to recreating the above result

# But based on your note below I will post a map_dfr() and tibble version
# (one of the map functions?) commented out in case we were allowed to
