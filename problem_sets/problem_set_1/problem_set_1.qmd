...
<i class="fas fa-user-secret fa-2x" style="padding-right: 6px;"></i>  Hints: 
  ...
  * See ***Preliminary Lesson one: R and RStudio*** for a definition of snake case.
:::
...
* [[0.50]]{.subscore} Modify this operation such that only "Functions that you may use in this assignment" are used and the output is produced with a single, *nested* code block with no global assignments (except `bird_counts`).
* [[0.50]]{.subscore} Modify this operation such that only "Functions that you may use in this assignment" are used and the output is produced with a single, *piped* code block that contains no global assignments (except `bird_counts`).
