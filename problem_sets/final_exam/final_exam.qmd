...
Your task with these data will be to:
...
* [[0.5]]{class="subscore"} Ensure that the class of your response is consistent with the wording of the question (e.g., if you are asked to return a "value" or "vector", ensure that the resultant object is not a data frame).
* [[0.25]]{class="subscore"} Ensure that your code follows the conventions of our course style guide.
* [[0.25]]{class="subscore"} Ensure that your code is as parsimonious as possible.
...
* [[1.0]]{class="subscore"} Ensure that all times in the data frame are formatted as ISO-8601 datetime values and stored as datetime objects. *Note: This includes the columns `crash_time`, `county_sunrise`, and `county_sunset`*.
* [[0.5]]{class="subscore"} Remove any transitive columns.
* [[0.5]]{class="subscore"} Arrange the resultant table from earliest to most recent collisions. 
* [[0.5]]{class="subscore"} Globally assign the name `collisions_date_fix` to the resultant object.
* [[0.5]]{class="subscore"} Remove the name `collisions` from your global environment.
...
* [[2.0]]{class="subscore"} Ensure that the four species are recorded as (with the first letter capitalized):
  * Black bear
  * Opossum
  * Raccoon
  * White-tailed deer
* [[0.5]]{class="subscore"} Globally assign the name `collisions_spp_fix` to the resultant object.
* [[0.5]]{class="subscore"} Remove the name `collisions_date_fix` from your global environment.
...
* [[2.0]]{class="subscore"} Modify the data such that these coordinates are provided in the correct columns.
* [[0.5]]{class="subscore"} Globally assign the name `collisions_coord_fix` to the resultant object.
* [[0.5]]{class="subscore"} Remove the name `collisions_spp_fix` from your global environment.
...
* [[2.0]]{class="subscore"} Make these data database-ready by normalizing (i.e., tidying) the data. Remember to follow all of Hadley's principles of tidy data! 
* [[0.5]]{class="subscore"} Store the resultant objects within a single list file globally assigned to the name `collisions_tidy`.
* [[0.5]]{class="subscore"} Remove the name `collisions_coord_fix` from your global environment.
...
Your new boss is demanding and more than a bit annoying (it only gets worse). For questions 7-12 you may choose to ignore some of your boss' memos. For these questions, we will maintain the four questions that have earned you the most points (i.e., best four out of six).
...
* [[1.5]]{class="subscore"} The columns are `year` and the name of each species;
* [[1.5]]{class="subscore"} The rows include the year followed by the total number of collisions for a given species, in a given year. 
...
We liked your table! Now our group is wondering if we can get a visualization to show any variation in the number of collisions per year. We want to show the total number of annual collisions by species and year. Please:

* [[1.0]]{class="subscore"} Complete all data processing steps (correctly) prior to piping the data into `ggplot()`.
* [[0.5]]{class="subscore"} Create a scatterplot with the total annual collisions on the vertical axis and year on the horizontal axis.
* [[0.5]]{class="subscore"} Use `geom_line()` to connect the points in the scatterplot.
* [[0.5]]{class="subscore"} Create a facet for each species.
* [[0.5]]{class="subscore"} Make sure that the y-axis is scaled so we can see annual variation in each facet. You can do that by setting `scales = "free"` (check out `?facet_wrap`). 
...
We have been gossiping about you are around the water cooler and we are all wicked impressed with the job you have been doing so far! Well, all of us except Chad, who is a bit of a tool -- Chad said "... yeah, but can they iterate?". Let's show Chad how it is done. Please:

* [[1.0]]{class="subscore"} Use iteration to generate four separate bar plots where each plot represents a single species.
* [[0.5]]{class="subscore"} Ensure that the horizontal axis of each plot represents years.
* [[1.0]]{class="subscore"} Ensure that the vertical axis represents the total number of collisions with a given species on a given year. 
* [[0.5]]{class="subscore"} Title each plot with the common name of the species.
...
that year. We have a suspicion that some police departments are under-reporting
collisions with these species. Please:

* [[2.0]]{class="subscore"} Verify whether every county reported at least one Opossum *or* Raccoon collision in 2017. 
* [[1.0]]{class="subscore"} If you find that some of these counties are missing, please provide me with a character vector of the offending county names.
...
* [[1.0]]{class="subscore"} Complete all data processing steps (correctly) prior to piping the data into `ggplot()`.
* [[0.5]]{class="subscore"} Visualize the data with a boxplot geometry.
* [[0.5]]{class="subscore"} Ensure that the horizontal axis of the plot is the number of crashes in a given year and the vertical axis is the names of the counties.
* [[1.0]]{class="subscore"} Arrange the plot from the county with the most crashes, across time, to the county with the least amount of crashes across time.
...
* [[1.0]]{class="subscore"} Complete all data processing steps (correctly) prior to piping the data into `ggplot()`.
* [[0.5]]{class="subscore"} Visualize the data with a stacked bar plot geometry.
* [[1.0]]{class="subscore"} Ensure that the horizontal axis is season, in the order Winter, Spring, Summer, Fall.
* [[0.5]]{class="subscore"} Ensure that the vertical axis is the total number of collisions, across years.
...
* [[0.5]]{class="subscore"} Subset to only vehicles traveling East or West.  
* [[1.0]]{class="subscore"} Subset the data to collisions that occurred within two hours of the sunrise time (Note: You may round the time to hour to address this question).
* [[1.0]]{class="subscore"} Classify the incidents that occurred for Eastbound drivers as "Into the sun" and those that occurred for Westbound drivers as "Away from the sun".
* [[0.5]]{class="subscore"} Generate a data visualization (of your choosing!) that illustrates whether more incidents occur for vehicles traveling into, or away from, the sun.
