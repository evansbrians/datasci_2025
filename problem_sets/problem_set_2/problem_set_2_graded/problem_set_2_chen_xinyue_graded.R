...
coqui <- read_rds("problem_set_2/coqui_counts.rds")
...
# I'm not sure if I'm understanding the prompt - aren't we violating the 
# first normal rule by further splitting each row?
...
ggplot() +
  ...
  geom_bar() +
  labs(
    title = "Number of observations per habitat type and site"
  ) +
  scale_y_continuous(
    expand = c(0, 0)
  ) 

# [[-0.25]] Incorrect: Did not modify the fill color of the bars.

# [[-0.25]] Incorrect: Did not set the upper limit of the y-axis to 170.
...
# The first rule is being violated. The primary key should be 
# transect_id instead of site, which depends on transect_id, therefore the 
# counts at different distances should be different variables in an observation
# and should be combined

# With that fixed, the third rule is still violated, since the coordinates 
# are transitively dependent on site instead of transect_id

# [[-0.50]]: More specificity is needed here -- this is a violation of the tidy
# data rule that every level of observation is placed in its own table
# (typically the third tidy data rule).
