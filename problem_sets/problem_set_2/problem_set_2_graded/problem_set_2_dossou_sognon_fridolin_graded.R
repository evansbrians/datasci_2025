...
# [[-0.05]] Code formatting: If the opening parentheses and the first argument
...
# The tidy data rule being violated in `coqui_long` is: "Each level of 
# observation forms a table."  

# I determined this by noticing that variables like `site`, `longitude`, and 
# `latitude` repeat across multiple rows, with 14 unique `transect_id` per site. 
# These location-related variables can be separated into a "sites_table" to 
# reduce redundancy.  

# A compound primary key can be defined using the combination of (`transect_id`,
# `date`, `distance_class`), with `transect_id` serving as a foreign key to l
# link observations to their respective sites.
