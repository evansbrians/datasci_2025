...
brood_x_parks <-
  ...
  pluck("address") %>%
  keep(
    ~ str_detect(.x,"([Pp]ark)|Zoo") %>%
    (!~ .x == "not_parks")
    )

# [[-0.75]] Incorrect:  
# * Since you are subsetting a data frame, this is a case for `filter()`.
# * When subsetting a data frame to a vector of values, use `%in%`.
# * Here, you are subsetting based on the value "not_parks" rather than the
#   vector `not_parks`

# [[-0.10]] If this had worked, the class of the resultant object would not
# have been a data frame.

# [[-0.10]] Code parsimony: The `(...)` in your regex was not necessary.

# [[-0.05]] Code formatting: Closing parentheses should be indented to the same
...
brood_x_park_summary <-
  problem_set_4_lifelines[["brood_x_parks"]] %>%
  summarize(
    n = n(),
    .by = c(address, species)) %>%
  arrange(
    desc(n)) %>%
  slice_max(n > 40)

# [[-0.25]] Incorrect: You did not have to group by both address and species.
...
problem_set_4_lifelines[["brood_x_parks"]] %>%
  semi_join(
    brood_x_park_summary %>%
      slice_max(brood_x_park_summary, n = 4)
    ) %>% 
ggplot() +
  aes(
    x = date,
  ...
  theme(
    ...
    strip.title = element_text(size = 14)
  )

# [[-0.70]] Incorrect:
# * Did not subset the observations to April
# * Did not map species to the fill color
# * Inside of `facet_wrap()`, you needed to set `scales = "free_y"`
# * Did not set the y-axis limits

# [[-1.25]] `[[...]]` is not among the functions that you may use for this
# assignment.
