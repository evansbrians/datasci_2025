...
1. **Download** the attached file "gis_in_r_2024.zip". 
...
I *strongly* recommend following the steps below at the start of each session (with the exception of this first session):
...
```{r, eval=FALSE}
install.packages(c("janitor", "arrow"))
```
:::
...
For packages that you use less than one day per week week, I suggest checking for new versions at the beginning of each session. For packages that you use more often, it is considered best practice to check for updates at least once per week. 
