...
The amount of memory that would be allocated to the latter is almost 1% that of the full object! As, such it again makes a lot of sense to reduce the memory allocated to the object prior to global assignment:
...
Another important tool for optimizing your code is **benchmarking**. Benchmarking tools can assess the time it takes for our code to run and the memory allocated to a given task. There are a few different ways this can be done, but here we will use the function `bench::mark()` (our current go-to method).

Let's use benchmarking to explore the memory usage and time of execution for our line of code that read in the `.csv` files in `automated_radio_telemetry`. We supply the code that we would like to benchmark and the number of iterations. Iteration in this context means repeating the same process a certain number of times in order to obtain accurate summary statistics:
...
The order in which we execute functions can also impact on the amount of memory used and processing times. 
