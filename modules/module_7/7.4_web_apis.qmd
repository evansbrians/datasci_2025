...
We can combine multiple variables of interest into a one-row tibble data frame with `bind_cols()` (remember the `bind_cols` function? We have not used it since Module 1!):
...
Instead, we can pipe the character vector into `set_names()` and refer to the character vector itself (with a placeholder) as the reference for those names:
...
This can be passed into our query and data extraction operations (not run, to save space):
...
... and, as such, we can query and extract the data for our movies iteratively (not run here, but just to save space):
...
The output of the above is a list of lists. Inside of the map function, we can assign the list to name, and bind the rows of the resultant list object:
...
As I add more filters, I like to use `str_c()` to connect my queries. Below, I continue with the above and filter to `taxon_name=Terrapene carolina carolina` (Eastern Box Turtle) `quality_grade=research`:
...
You might have noticed that each of the returns above returned 30 rows, regardless of how we subset the records. This is the default number of returns from the iNaturalist API. We can increase this up to 200 (any higher requires an API key) with the `per_page` parameter:
...
* `httr::content`: Extract content from a web API request
* `httr::GET`: Request information from a URL
...
* `stringr::str_replace_all`: Replace a multiple instances of a given pattern in a string with a new pattern
:::
