...
The methods that I have demonstrated in this course are built to run effectively and smoothly for most datasets. In many instances, I have purposefully shown methodology that works for big data, though performance improvements for the small teaching datasets that we used were often negligable or nonexistent. Herein, I will address how to use our course methods when confronted with (fairly) big data. We will cover:
...
You will also get a bit more practice with benchmarking, writing custom functions, and iteration with *purrr* (including a couple new mapping functions)!
...
3. Open the script file [big_data.R]{.mono} (Windows: [Ctrl + O]{.mono}; Mac: [Cmd + O]{.mono}).
...
* Will return a value of `FALSE` if the vector *does not* contain duplicates;
...
Let's have a look at the `messy_birds` data frame:
...
As such, these data should be split into two different tables. We can combine the separated data frames into a single object with `list()`. Let's do so and globally assign the resultant object to the name `tidy_birds`:
...
The object that we created above is a list of tibbles and, given that a data frame is a restrictive class of list, a list of lists.
...
Let's generate a unique character vector of species codes in `messy_birds`:
...
Let's take a look at the structure of our nested list:
...
Of course, as I am sure that you have discovered, the trouble with lists is that they can be difficult to work with! Although the *purrr* package has made this process a lot easier, it can still be challenging.
...
That is, of course, not a very parsimonious solution. When using `map()` previously, we often supply a character vector of names to iterate across, but remember that we can iterate across list items as well! Let's `map()` across the tibbles in our list column. Below, I modify each item in our list column, subsetting each data frame to where the mass is less than 20:
...
Notice that we have not subset the larger data frame, but the dimensions of each tibble have changed. Only the tibble in the row associated with "Carolina Wren" contains any rows (`<tibble [1 × 3]>`).
...
... but this is even less parsimonious than before!
...
This is better, but it was not necessary to assign `n_records`. Instead, we can replace `mutate()` with `filter()` to remove rows where the list column does not contain any records:
...
* If we want our function to work on any nested data set, we must include the data frame as one of the arguments of our function. We will refer to this in our custom function with the variable `.data`.
...
:::{.now_you}
<button class="accordion">{{< fa user-circle size=2x >}}&nbsp; Write a custom function that will filter a nested data frame based on a given list column and predicate. Globally assign the function to the name `filter_nested_frame`. *Click this button to see my solution!*
...
We have discussed primary keys and foreign keys considerably within the context of joins. A **primary key** is a column in which each value uniquely identifies a given observation (row). A **foreign key** is a column in which each value represents the primary key of a different table. These keys allow us to query data across tables in a dataset and thus give us database-like functionality.

::: mysecret

{{< fa user-secret size=2x >}} [An Excel file is not a database!]{style="font-size: 1.25em; padding-left: 0.5em;"}

Although we can follow the **tidy data** rules to structure a dataset as though it was comprised of database tables, a spreadsheet is not a true database. A database unites the data into a hierarchical object that provides a consistent, safe method for querying across tables.
:::
...
My method is to use the function `str_rand_strings` from the *stringi* package (the package that *stringr* is built upon) to generate random strings. `stri_rand_strings()` contains the following arguments:

* `n = ...`: The number of observations;
* `length = ...`: An integer of the desired number of symbols for each string;
* `pattern = ...`: A character vector specifying character classes to draw elements from. The default argument is "[A-Za-z0-9]", which specifies any uppercase or lowercase letters and any number 0 through 9.

Let's generate a vector of three random strings (`n = 3`) in which each string contains two characters (`length = 2`) and each character value can be any number 0 through 9 (`pattern = "[0-9]"`).
...
We can add lowercase letters to include in our strings:
...
We can use the character vector `letters` (a built-in constant in base R) to see that we are now up to 1,296 potential combinations with two symbols (which still does not leave us a lot of options):
...
Let's add uppercase letters as well (the default `pattern` argument):
...
We can see that we now have 3,844 unique combinations with just two symbols by including uppercase `LETTERS` (another built-in constant in base R) within `expand_grid()`:
...
Let's create a key for `messy_birds` where each value contains 4 symbols and assign the resultant object to the variable name `id` in `messy_birds`:
...
* Using the above, create a function that will add a primary key to any data frame;
* The arguments of the function should be a data object and the number of symbols to use;
...
Because the strings are generated randomly, a potential danger to our method is that it is possible to generate duplicate key values. To test this, we must determine whether any of the key values are duplicated. We can use our custom function `contains_duplicates()` (from the start of our lesson) to test this:
...
Our key does not contain duplicate values, so it is safe to use! This is not surprising because there are almost 15 million potential values when we use 4 symbols and our data frame has only 5 rows. The probability of generating a duplicate for these data is nearly 1 in 3 million! This is good, because we *really* do not want to add a duplicate key.

Of course, it *is* still possible to generate duplicate key values. As such ...

:::{.now_you}
<button class="accordion">{{< fa user-circle size=2x >}}&nbsp; Working in the `custom functions` code section, modify the function `add_key_to_frame` such that:
...
As we have verified that we have a working function, let's add an `observation_id` primary key to `messy_birds` and globally assign the name `messy_birds_keyed` to the resultant object:
...
The above verifies that we have successfully created our primary and foreign keys. So let's split the data based on the levels or observation (the third tidy data rule) and bask in our tidy glory:
...
If we wanted to store the data above using the methods that I have taught in this course thus far, we would  have created a list and stored it on our hard drives as a [.rds]{.mono} file:
...
As I have previously described, [.rds]{.mono} files have the limitation that these objects can only be read by R. Additionally, when we read in an [.rds]{.mono} file, we read the whole file at once into our memory. This is not a great solution for big data!
...
* The function `SQLite()` specifies that the new database will be an SQLite database.
...
:::{.now_you}
<button class="accordion">{{< fa user-circle size=2x >}}&nbsp; Use `map()` to add both tables in `my_tidy_list` (`captures` and `life_history`) into our database in a single step (use `overwrite = TRUE`).
...
The real power in a database, however, comes from queries across tables (i.e., joins). To subset the list item `captures` in `my_tidy_list` to those in which the diet is "insectivore" (from the `life_history` table), we would use:
...
We would not see any performance improvements with data that are this small (in fact, the performance is considerably worse!). Below, I use the *bench* function `mark` to evaluate the operation above completed with a list stored in memory...
...
2. Converted *dbplyr* code to SQL code
...
You might have noticed that, once the data are written to the database, the read operation is much faster:
...
But notice that the memory required to complete the operation above is considerably lower!
...
*dbplyr* helps a ton once you have read in a file. Knowing a bit of the SQL language, however, is necessary if you wish to modify the database itself. For example, here is how to delete records for a given `tag_id`:
...
If SQL is new to you, I recommend learning by printing the query every time you use *dbplyr*:
...
To append an SQL table, we use the DBI function `dbAppendTable()`:
...
Our final topic of this lesson will be the simplest -- the *arrow* package. I use this package a lot for big data that are stored as [.csv]{.mono} files. Like the database operations above, the advantage of this package is that files are not stored in your memory until it is necessary to do so.

With *arrow*, we establish a connection with a [.csv]{.mono} dataset with the function `open_csv_dataset()`:
...
*Note: Some operations available in dplyr are not yet available with arrow. Above, I could not use `slice_max()` on grouped data and needed to specify `with_ties = FALSE`. More and more tidyverse operations are being integrated into arrow, so this will likely change in the future.*
...
* **Predicate**: A logical test (i.e., it will return a Boolean TRUE or FALSE).
