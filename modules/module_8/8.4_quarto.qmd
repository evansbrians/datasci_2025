...
* **Ordered lists** should be used when the sequence of events matters and list items are typically preceded by numbers or letters. For example, you might use an ordered list to describe the steps in a process or the rank of list items.
* **Unordered lists** (like this one!) are used to communicate conceptual groupings in situations in which their sequence is not important abd are typically preceded by bullet points. Unordered lists are useful for communicating information quickly and sorting or displaying the hierarchy of list items.
...
We can define a style across our entire document by adding a CSS code chunk. Because this modifies the CSS of the entire document, I always do this at the beginning of my script. Similar to adding an R code chunk, the chunk is defines by opening and closing triple backtick quotes. Inside of curly braces you specify that you are adding CSS.
...
```
````
*Note on the above: We used* `echo = FALSE` *because we do not want show our reader our CSS!*
...
In the above, I defined a style class called `.tara_style`. I added my style parameters inside of curly braces. Now, I can refer to my custom style class using:
...
You can class add a style to a predefined class:
...
We have just barely scratched the surface of what you can do with Quarto. For the sake of brevity, we have only included the basic elements to get you started with using Quarto. For example, we typically define our CSS with an external file and also tend to include JavaScript and HTML within each Quarto document that we produce! We strongly recommend reading the Quarto documentation at [this link](https://quarto.org/docs/guide/){target="_blank"} ... you will even learn how to create hyperlink (`[this link](https://quarto.org/docs/guide/){target="_blank"}`)!
