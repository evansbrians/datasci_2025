...
To create the above, you would use keyboard shortcuts or buttons to define the *italic* and **bold** text. Behind-the-scenes, you are assigning tags to those literal symbols that define how those words are presented. For WYSIWYG markup like Microsoft Word documents, all of the code is hidden from the user by default.
...
Notice in the above that, though we clicked the "Source" button when we set up the document, it is still listed as `editor: visual`. This means that the document will open with the Visual editor each time we open it. Let's change that:
