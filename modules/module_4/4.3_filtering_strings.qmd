...
Regex gives us remarkable flexibility when dealing with messy character class columns and I consider it to be one of the most important tools in an effective data manager's skillset. In this lesson, we will explore literal characters, metacharacters, and the *stringr* package (**core tidyverse**). This lesson includes a video tutorial (runtime -- 42:31) that describes how to filter data with regex and a *compendium* intended to serve predominantly as a reference. The compendium does include *some* new material and I have included many "Now you!" exercises in the compendium to give you additional practice with regex. In completing this lesson, you will learn how:
...
::: panel
In the video portion of this lesson, we will explore the file [my_covid_garden.csv]{.mono}. The data are formatted as a tibble data frame object comprised of the following variables:
...
3. Open the script file [filtering_strings.R]{.mono} (Windows: [Ctrl + O]{.mono}; Mac: [Cmd + O]{.mono}).
...
* In my `aes()`, I should have placed `reorder()` on its own line (based on the course style guide). Likewise, for `filter(year(...) == ...)`, `year()` should have been placed on its own line.
...
{{< video https://www.youtube.com/embed/_CwBKTPw41U >}}
...
The function `str_detect`, in the *stringr* package, is a logical test of whether the values in a given vector matches the RHS *pattern*. 
...
We see that both values evaluated to `TRUE` because the string contains the symbol `"r"`.
...
... or use the `negate = ...` argument of `str_detect()` (see `?str_detect`) to negate the resultant logical vector:
...
blue whale" and "black-throated blue warbler"
```{r}
str_detect(
  c("big bluestem",
...
<button class="accordion" style = "margin-bottom: 18px">Using the search pattern "bird" or "birds", evaluate this statement such that both Nina **and** Maude's reference to birds evaluate to `TRUE`.</button>
...
<button class="accordion" style = "margin-bottom: 18px">Using the search pattern "bird" or "birds", evaluate this statement such that Maude's reference to birds evaluates to `TRUE` and Nina's evaluates to `FALSE`.</button>
...
<button class="accordion" style = "margin-bottom: 18px">Using the search pattern "bird" or "birds", evaluate this statement such that Nina's reference to birds evaluates to `TRUE` and Maude's evaluates to `FALSE.`</button>
...
You can search the start of a string using the `^` metacharacter (outside of square brackets!). For example, in the character vector below, let's detect "gray" or "gray wolf", not "stingray":
...
<button class="accordion" style = "margin-bottom: 18px">Detect the value that begins with the number "1":</button>
...
<button class="accordion" style = "margin-bottom: 18px">Detect the value that ends with the number "1":</button>
...
For example, the following can be used to detect "gray" or "grey", not "stingray" or "graying":
...
To detect values that start and end with some symbol within the values one through nince, we have to add anchor metacharacters to narrow our search:
...
Likewise, we can specify a range of values for the first digit in our two digit string. Let's evaluate which values are between 10 and 29:
...
The `*` repetition metacharacter searches for symbols that may be absent or  repeated an unlimited number of times (match the gray whale and wolf):
...
Metacharacters are useful, but sometimes we want to refer to a metacharacter as a literal character. We can do so by placing a double backslash `\\` prior to symbol. 
...
If we tried to test for the pattern "160 $" within the string, without an excape, all values evaluate to `FALSE`:
...
A US phone numbers are often formatted as "1-212-736-5000" (as above), "1 (212) 736-5000", or "+1 212-736-5000".
