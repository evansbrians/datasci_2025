...
This did not work either! This is because R was trying to sort the data based on the one-value character vector "sepal_length", not the *variable* `sepal_length`. We can address this by again using `{{{ ... }}}` to insert our variable into the function:
...
I should also reveal that, although we have used `desc()` within `arrange()` throughout the course, this operation can be simplified! If the variable that you want to `arrange()` is numeric, you can use the `-` operator in place of `desc()`
...
Note that `desc()` works on character vectors, but `-` does not!
