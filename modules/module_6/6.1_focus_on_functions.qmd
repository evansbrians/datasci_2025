...
Custom functions are important tools for our data science skillset. Writing custom functions:
...
To illustrate this, let's make a simple function and assign it to the name `multiple_by_two`:
...
We supplied the formals of the function as the value 5. That value was passed to the body of the function and thus any time an `x` appeared, the variable was replaced by the value 5.

### R function basics, applied

Let's create a function with some real data. Consider that you are given the task of calculating the average `sepal_length` for one of the species in the `iris` data:
...
```
*Note: In the second two operations, I do not include `spp = ...` because it is not necessary to provide the name of formal if it is the only argument in the function.*
...
This behavior is actually the result of the curly braces (`{...}`). When you wrap an expression in curly braces only the last expression is passed to the global environment. This is why the body of a function is typically wrapped in curly braces. Curly braces are not necessary if a function only occupies a single line, for example our function `multiple_by_two`:
...
The above works, but naming your variable differently based on context (e.g., as a formal vs. a variable in a data frame) makes your code difficult to read/interpret. So can we name our formal `species` instead? Let's see what happens:
...
It did not! Once again, `sepal_length` was not found due to R's scoping rules. What happens if we place the variable in quotes?
...
This did not work either! This is because R was trying to sort the data on a one-value character vector, `"sepal_length"`. We can address this by again using `{{ ... }}` to insert our variable into the function:
