...
* **Maps in purrr**: The purrr package makes iteration easy!
...
Although the values of the vector are 6 through 11, the indices (i.e., positions) of the vector are the values 1 through 6. For example, the value at the second position of `my_vector` is 7 and the values at the second and third positions of `my_vector` are 7 and 8.
...
With tidyverse functions, we can extract vectors or lists to a single position with the function `pluck()`.
...
For the above, however, I would typically use `pull()`, as that function is reserved for data frames, thus its usage is more specific:
...
If we instead wish to subset by row position, we use the `slice` family of functions in *dplyr*:

```{r}
slice(portal, 1)
```

... which also works with multiple positions:
...
Reading (and modifying) the above can be challenging. As objects become more deeply nested, the challenge grows! For example, what if `portal` was a list item in a large list? In the code block below, I nest `portal` inside of a list and complete the same operation:

```{r}
list(portal)[[1]][1:5, ][[6]]
```

Although the tidyverse version of the operation requires more functions, the readability of this code block is greatly enhanced:
...
Indexing is useful, especially with iteration (we will get there in a moment!), but numbered indices do a very poor job of communicating what is happening in our scripts. As such, I recommend using assigned names whenever possible. 

We can set names to the positions within an atomic vector using the *rlang* function `set_names()` (*Note: This function is made available to use with `library(tidyverse)`*):
...
I often choose to instead use `tidyselect:::select` when subsetting from a list (because quotes are not necessary; but note that this operation is a subset, not an extraction):
...
When we first learn R, our code rarely includes iteration (*at least knowingly*). Our scripts are long, repetitive, and often filled with errors. If you have to remember where to put your comma 100 times in a single script, chances are you are going to get it wrong more than a few times along the way.
...
If we wanted to calculate mean hindfoot length of the White-throated woodrat (*Neotoma albigula*) in base R, we do so with indexing as:
...
In the tidyverse, we could use the filter function to do this:
...
Ack! There are 29 species. That is a lot of repetitive code. There must be a better way ...
...
Within our sequence statement, this would be written as  (do not run):

```{r, eval = FALSE}
for(
  i in seq_along(target_vector)
) ...
```
...
To check if your for loop worked, I *strongly* suggest assigning your variable (which here is `i`) to the global environment and evaluating what happens in a single iteration:
...
Our for loop worked, and generated the correct results, but we actually could have completed the same operation with:

```{r}
target_vector + 1
```

Let's look at some more useful applications ...

### For loops applied: A population model

For loops can be very useful when modeling things like population growth. In this section we will use for loops to calculate the populations sizes using two common population models.
...
Let's calculate the population over 10 generations.
...
Next, we will set the value of the population at the first time step:
...
It would take a lot of repetitive coding to do this with *every* species in the data set. In data science, this is often referred to as a split-apply-combine problem -- we split a data object into parts, apply a function to each part, and combine the resultant values into a new object.
...
{{< video https://www.youtube.com/watch?v=ljXxhRq_pdg >}}
...
Writing `map()` functions is *much* more straightforward than for loops in that you only need to provide:
...
Like a for loop, we can map by position in a vector. Below, I reconstruct our *for loop by index* above with a `map()` operation -- this will be followed by an explanation.
...
And then apply a function to the subset:
...
There are three ways to specify the formula in a *purrr* map. Above, we specified the formula with `function(x)`. 
...
When using `function(x)`, R searches for `x` as a variable in your search path. As we have seen previously, the `~` operator is used to specify a formula. The `.` in `.x` defines the values in our vector as a variable within a formula:
...
file.path(
  "function_tables",
  "functions_6.2_iteration_with_purrr_and_for_loops.csv"
) %>%
  read_csv() %>%
  kableExtra::kable(
    align = c("c", "c", "l")
