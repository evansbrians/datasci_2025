---
title: "3.4 Introduction to summarizing"
format:
  html:
    embed-resouces: true
    self-contained: true
...
This lesson includes a video introduction to summarizing (from my course on GIS in R) and a written tutorial that reviews and adds to the video content. If you are a little exhausted from the *super* long subsetting lesson (I know I am!), this one is much shorter because summarizing is very straightforward! In completing this lesson, you will learn how to:
...
*Note: Leaving data grouped is VERY dangerous! I always suggest removing an grouping after the grouping is no longer necessary.*
...
*Note: Although the above can be produced with `tally()`, I typically use this version, as it is more flexible (because I can add more summary statistics other than simply counts).*
...
But notice the warning message we received ... the data are still grouped! By default, `summarize()` will drop the last group used -- when we supply two or more groups, the resultant data will be grouped by default. If we summarize based on a single group, we do not have to think about it. 
...
Thanks to a fairly new **experimental** feature, `.groups = "drop"` (May 2020), we can now remove the grouping structure from the data without the additional `ungroup()` function: 
...
So is `group_by()` a relic of the past? No!!! A difference between `group_by()` a `.by = ...` is that we can calculate and assign a new column during the `group_by()` process (thus `group_by()` includes a mutation). We cannot (currently) do so with `.by = ...`. As such, this works ...
...
... but this does not we currently cannot do so with `.by = ...`
