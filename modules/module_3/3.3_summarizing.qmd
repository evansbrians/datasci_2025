...
* I do not conform to the course style guide in this video! The biggest violations are:
    ...
    * `select()` and `c()` should not have been on the same line. Additionally, because `c()` had three arguments, each argument should have been placed on its own line.
    * `group_by()` and `year()` should not have been on the same line. 
    * `group_by()` and `month()` should not have been on the same line. 
    * I should have put each argument of `month()` on its own line.
    * When I have two or more arguments in `aes()`, and they both contain `=`, they should have been placed on their own lines.
    * `summarize()` and `max()` or `min()` should not have been on the same line.
