...
* I do not conform to the course style guide in this video! The biggest violations are:
    * This was back when I used single quotes for everything. I now much prefer using double quotes.
    * I did not use hanging parentheses.
    * I did not include a leading and trailing space with the `*` infix function.
    * I should have hit enter after the opening parentheses of `mutate()` and `transmute()` when the functions arguments are placed on multiple lines. Doing so would have ensured that the arguments of the function are indented two spaces relative to the function call.
    * `as.numeric()` and `c()` should not have been on the same line. Additionally, because `c()` had three arguments, each argument should have been placed on its own line.
    * When `mutate()` has only one argument, but that argument contains another function, the argument should have been placed on its own line.
