---
title: "3.3 Introduction to subsetting"
format:
  html:
    embed-resouces: true
    self-contained: true
...
* Enhancing column selection with `everything()` and `where()`
...
::: panel
In the video portion of this lesson, we will explore the [*.rds]{.mono} file [weather_tidy.rds]{.mono} that you wrote to your hard drive at the end of ***3.1 Tidy data review***. The data are formatted as a list object  comprised of the following tibble data frames and variables:
...
* My indentation was off for some of the multi-line functions. I should have hit enter after the opening parentheses to ensure that the arguments are indented to spaces relative to the function call.
...
Now we are going to add our data for this lesson ... let's do this as a two "Now you!" challenges:
...
**Remove a single column**: A negated selection will return all columns except for those specified by the negation operator, the `!`. Here, we will remove the column `band_number`:
...
Up to this point, we have used the extraction operator, `$`, to extract a list item from a recursive objects (lists and data frames). For example, in the below, I extract the character vector assigned to `foraging` and subset the vector to unique values:
...
The `slice()` functions becomes a very powerful tool for data exploration and wrangling when used in conjunction with `arrange()`. For example, perhaps we want to see the five lowest mass values in the `measures` data:
...
We can use *tidyr* (core tidyverse) function `drop_na`, with no arguments, to remove all rows that contain an [NA]{.mono} in any variable (use carefully!):
...
Our final subsetting operation of this lesson (we will go in-depth into subsetting in ***Module 4: Focus on subsetting***) is the function `distinct()`. Distinct is used to subset the data to unique rows -- in other words, any rows that are duplicates will be removed.
...
* `.Primitive, ==`: Prefix relational operator (is equal to)
...
* `Primitive::is.na`: A logical test of whether a value is NA
...
* `base::%in%`: Infix logical operator that tests whether the vector on the left-hand-side (LHS) is within the vector on the right
...
* `tidyselect::where`: Used in combination with select, this selects columns that satisfy a condition
:::
