---
title: "3.5. Introduction to joining data frames"
format:
  html:
    embed-resouces: true
    self-contained: true
...
* Conduct mutating joins with `full_join()`, `inner_join()`, `left_join()`
* Conduct filtering joins with `anti_join()` and `semi_join()`
* Joining tables with **compound keys**
* Joining tables that are at the same level of observation
...
::: panel
In the video portion of this lesson, we will explore two files -- [weather_tidy.rds]{.mono} and [weather_tidy.rds]{.mono}. 

**[tidy_weather.rds]{.mono}**: These data are formatted as a list object comprised of the following tibble data frames and variables:
...
**[messy_birds.rds]{.mono}**: These data are a toy dataset of bird bading data and represent an untidy tibble. Variables include:
...
* [count_id]{.mono}, character: The primary key for a unique capture record.
...
Now we are going to add our data for this lesson ... let's do this again as a two "Now you!" challenges:
...
The connections above represent common columns that are use to connect information across data frames. When establishing connections between tables, it is important to consider which table is the target of the join and which table is the source of joining information. We can consider these two tables as:
...
Let's join the source table `visits` (primary key: `count_id`) to the target table `counts` (foreign key to `visits`:  `visit_id`). We supply:

* The name assigned to the target table;
* The function `left_join()`;
* The name assigned to the source table;
* The argument `by = `, which specifies the name of the key column upon which the join is based.
...
A new feature to the join functions in *dplyr* is the `join_by()` function (introduced in January 2023). It is supplied in the `by = ...` argument argument and, within the parentheses of `join_by()` we provide a logical test to define the joining variables:
...
If we are not interested in *all* of the columns of the source table, it is best practice to subset the columns being joined. For example, perhaps we're only interested in knowing the date in which a count occurred. We can address this by subsetting the columns during the join process:
...
Let's fake the issue. Below, I generate two tables that are both represent the "species" level of observation. I arrange the columns of each dataset by life history guild and then species within the guild:
...
A join is a (fairly) safe option for addressing this problem. In this case, the primary key of `foraging` is also primary key of `diet`. As such, the foreign key of `foraging` is, of course, also the foreign key of `diet`. As such, the order of tables to supply to the join (target and source) do not really matter:
...
I will fake the issue again by making two tables of counts, one for "GRCA" (Gray catbirds) and the other for "NOCA" (Northern cardinals). For each table, I'll include the `site_id`, `date`, `spp`, and `count`:
...
Notice that the two joins produced very different results. The number of observations (rows) is determined by the target table. When there was no match in the source table, an `NA` (missing value) is returned. I'm not overly concerned with the `NA` values, but the different number of rows are a major issue given that the level of observation is equivalent for the tables. This is the sort of situation in which we might face data loss. To address this, let's return to our classes of mutating joins ...
...
::: panel
**Important!** Primitive functions as well as functions in the *base* and *utils* packages, are loaded by default when you start an R session. Functions in *dplyr*, *tibble*, *purrr*, *tidyr*, and *tidyverse* are loaded with `library(tidyverse)`.
...
* `dplyr::arrange`: Sort the rows of a data frame by the values in a variable
...
* `dplyr::left_join`: Join a table on the left to a table on the right by a common variable (typically called a **key**)
