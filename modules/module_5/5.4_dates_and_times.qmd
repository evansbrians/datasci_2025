...
Nothing strikes fear in a data manager more than poorly formatted dates ("... why on Earth did you think that was an appropriate date format to use?"). In the base R universe, there is a layer for hell dedicated to spending eternity fixing all of the various date formats that our technicians use in the field. Luckily, a little bit of regex and *a lot* of the *lubridate* package (now a member of the **core tidyverse**) has made date problems a thing of the past! This lesson includes a video (runtime -- 19:34) and a written tutorial. The written tutorial for this lesson includes very little new material (specified with the heading "new content:"). Herein, you will learn:
...
In this written portion of the tutorial, we will again attach the core packages in the tidyverse to our current session read in and globally assign [dates.rds]{.mono}:

```{r, message = FALSE}
# R script for the subsetting vectors compendium
...
The international standard format (ISO 8601) for dates is yyyy-mm-dd. Converting a character date in ISO 8601 format is simple with *lubridate* with `as_date()`:
...
This is the case whether the date column is of class date-time or character (though *do* note the warning message below!):
...
The data in this table is the same as our `good_dates` data set, but here I'm expressing the date in three different European date formats.

Let's see what happens when we use the `as_date()` function on this object to get a true date column:

```{r, error = TRUE}
european_dates %>% 
  mutate(date = as_date(dot_yyyy))
```
...
Let's look at out `us_dates` object: 

```{r}
us_dates
```

We can convert the date format [mm]/[dd]/[yyyy] to standard dates with `mdy()`:

```{r}
us_dates %>% 
  mutate(date = mdy(slash_yyyy))
```
...
```{r}
us_dates %>% 
  mutate(date = mdy(abbr_month))
```

#### Mixed dates

I know that it might sound crazy, but I have worked with character vectors that represent dates in *both* in European and US date formats (in the same table!). The best option when this is the case is to use regex to try to detect the format that the dates are stored in.
...
Transitive dependence is *super* common with date, time, and date-time variables. I find typically run into this problem with folks who store and explore their data using Microsoft Excel. This often looks something like (at best):
...
There are *loads* of functions like this in the *lubridate* package! For example, we can extract the month number:
...
*Note: Unlike most tidyverse packages, which tends to be conservative regarding the amount of functions, the lubridate package currently uses a different function for each time increment. Be sure to download the cheatsheet for the package ... it is easy-to-use and handy!*
...
* `lubridate::hour`: Extract the hour-of-day from a date-time or time class object or a properly formatted date-time character vector or a properly formatted date-time character vector.
