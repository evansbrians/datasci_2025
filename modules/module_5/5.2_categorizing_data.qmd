...
* [longitude]{.mono}, numeric: The longitudinal coordinate of where a bird was found (EPSG 4326) ... note that longitudes in the Western Hemisphere should always be negative!
* [latitude]{.mono}, numeric: The latitudinal coordinate of where a bird was found (EPSG 4326)... note that latitudes in the Northern Hemisphere should always be positive!
...
To see how this works, let's start with a tibble with a single column that contains integers with the values 1 through 5:
...
The easier of the two is `right = ...`, so we will start there. If `right = TRUE`, the default, this means that the interval is open on the left (lower boundary) and closed on the right (upper) boundary. This is the default behavior and is why our resultant classes were noted as `(x,y]`. Conversely, if `right = FALSE`, the interval is closed on the left and open on the right.  
...
The tidyverse package *stringr* offers easy-to-use tools for finding information about strings, modifying strings, and replacing them. Using *stringr* functions, we do not need to supply all of the characters in a character vector -- we are searching for matches in only a portion of a string. As such, this process is known as **partial string matching**. Herein, I will focus our attention in this section on stringr tools that I use most frequently when I am working with spatial objects.

### Detecting strings

We use the function `str_detect` to determine whether a string matches a provided pattern. This function is a type of logical test, thus the output of the test will be `TRUE` or `FALSE`. We supply a character vector and the pattern to search for. The arguments for `str_detect()` are:

* `string = ...`: The character vector (or assigned name) that we would like to modify;
* `pattern = ...`: The pattern to search for.

In the below, `str_detect()` searches the string for the symbol "r" at any position in the string "gray":
...
We can use `str_detect()` to detect multiple symbols:
...
... but `str_detect()` evaluates symbols in the order that they appear. For example, the `str_detect()` function below is testing whether the pattern "gry" is contained within the character string "gray":
...
Although the characters `g`, `r`, and `y` all occur in the string `gray`, they do not appear in that order -- thus, the statement above evaluates to `FALSE`.

We can also apply `str_detect()` to character vectors that contain more than one value. Let's conduct a logical test to search for the pattern "rr" in `example_df$fruit`. Below, I add a new column, `test_fruit`, which is the results of the test.
...
If we wanted to search for values in `fruit` that *do not* contain the pattern "rr", we can precede `str_detect()` with the negation operator, `!`:
...
Likewise, I can subset `example_df` to rows where `fruit` does not contain the pattern "rr" with:
...
A search pattern can refer to literal characters or a combination of literal and metacharacters. A **literal character** is a symbol representing the value of a single symbol within a string. A **metacharacter** is a symbol that has a special meaning during pattern processing. Combined, literal and metacharacters make up **regular expressions** (regex), which are sequences of characters that describe search patterns. In the examples above, I only provided literal characters. There are a number of metacharacters that can enhance our ability to efficiently search for patterns. We will only cover a couple of these in this class, but it is a key topic of our <a href="https://smconservation.gmu.edu/programs/graduate-and-professional/ecological_data-ol/" target="_blank">introductory class on Data Science</a>!
...
Searching for character vectors using the `%in%` operator can sometimes be a bit cumbersome. For example, if we wanted to subset `example_df` to just rows where the fruit is "banana", blackberry", or "boysenberry" we would typically use:
...
That can be okay when there are so few choices, but there is *sometimes* a better way! 
...
A big part of working with a GIS is making maps. The rules that apply to field names when wrangling data are, however, quite different than those associated with reporting data. Does your boss/advisor/dog want to see your results in snake_case? No, they do not.
...
file.path(
  "function_tables",
  "functions_6.1_categorizing_data.csv"
) %>%
  read_csv() %>%
  kableExtra::kable(
    align = c("c", "c", "l")
