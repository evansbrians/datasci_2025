...
It is often necessary to repeat the same process on multiple variables. This can lead to code that is unnecessarily repetitive, long, and thus difficult to read. More importantly, this can also mean that edits made to a repeated process must be made for *each* repetition separately. Luckily, as of 2020, the *dplyr* package provides us with tools for iterating a process across multiple variables in a data frame -- this is known as **column-wise iteration** (often referred to as colwise iteration, in short). In this video lesson (19:02) and compendium tutorial, which is a review except for the sections denoted by **New content**, we will explore some new *dplyr* tools and you will learn how to:
...
* Although I describe that `transmute()` was superseded with a good bit of melodrama, I have since decided that there are many instances in which I prefer the use of the function.
* I do not conform to the Course Style Guide in this video! The biggest violations were:
  * I much prefer using double quotes rather than single quotes
  * I do not include hanging parentheses with `mutate()` or `select()`
