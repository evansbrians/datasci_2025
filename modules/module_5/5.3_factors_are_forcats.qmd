...
::: panel
In this lesson, we will explore the file [forcats_data.rds]{.mono}. The data are formatted as a list of the following tibble data frames:
...
In this written portion of the tutorial, we will again attach the core packages in the tidyverse to our current session read in and globally assign the files in  [forcats_data.rds]{.mono}:
...
# Read in the data for this lesson and assign the name of each list file
...
A factor is stored an integer in your computer's memory. We can verify this with `typeof()`:
...
Because printed output `str()` truncates the factor levels, it is often useful to explore a factor by using `levels()` to generate (and print) a character vector of factor levels:
...
We can see that, although the character vector that we converted to a factor starts with "small", the levels are arranged as "large", "medium", then "small". That is because levels are assigned alphabetically by default. That is most likely an issue with this factor, as it is clearly **ordinal variable**. We can address this by specifying the order of the levels with the `levels = ...` argument of `factor()`:
...
When creating a factor variable, we often need to repair misspellings or reclassify character values. We can do so *before* creating the factor (or during factor creation, as described in the sections below).
...
If we plot the resultant data, we might notice that the variable, which should probably be an ordinal factor, is arranged alphabetically:
...
The above, by far, my choice for modifying completing this problem. It is the most parsimonious of the methods that we used above and does a great job of communicating which values in the character vector are being modified.
...
If we just want change one of the unique values and generate a factor, we can do this in one step:
...
The above is a great option if your resultant object has only a few factor levels *and* if the original data has only only have a few factor levels to convert:
...
If you have a strong sense of the data that you are modifying, you can simplify the above by collapsing *all* of the unique values *not* specified. For this, you use the `other_level = ...` argument:
...
We can supply factor levels *while* creating a factor by simply providing a vector of the factor levels in the specified order:
...
Let's apply this to real data. To generate the plot below, I subset the data to the species (`common_name`) with at least 100 observations, count the number of observations per species, and plot the data:
...
Finally, we can also reorder factors in a tibble based on another variable using `fct_reorder()`.
...
As we might expect, the levels are mapped to their integer values in alphabetical order.
...
This plot is *just okay*. It's pretty easy to distinguish the species that were encountered the most often, but does not do a great job at visualizing the distribution of encounters. Instead of `fct_rev()`, to arrange the levels in reverse alphabetical order, we can use `fct_reorder()` to arrange the levels by the count column, `n`:
